...
from function.page.contact_page import *     #引入自定义的公用操作包
        ...
        for case in cases:
            ...
            try:
                ...
                if dbrc[0] == False:   #执行SQL语句出错的处理
                   print(dbrc[1])
                self.rzContact.ranzhi_contact_add(case)      #调用公用函数执行产品添加
                rc = self.rzContact.ranzhi_contact_add_v(case)         #调用公用函数执行登录后验证
